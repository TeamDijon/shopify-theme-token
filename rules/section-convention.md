...
- Base settings (Layout + Appearance) — see `.context/docs/schema-conventions.md#section-base-settings`. Required for merchant-addable standard sections; optional for specialized or static sections. Section settings are `content_width` + `block_rhythm` + the `custom_color_scheme` gate + `color_scheme` (see `.context/docs/schema-conventions.md` § Color-scheme override); no `layout` setting (section is always a bleed grid — see `.context/docs/theme-root.md`)
