...
7. **Honor user preferences.** `@media (prefers-reduced-motion: reduce)` and `@media (forced-colors: active)` are already wired into `layer-reset.css`; per-component CSS should respect them.
...
Strings live under `accessibility.*` in locale files (see `.context/docs/locale-conventions.md`). Render via `| t`; visually hide with the `.sr-only` utility defined in `assets/layer-utilities.css`.
...
- **Reduced-motion + forced-colors handling** — `assets/layer-reset.css`
