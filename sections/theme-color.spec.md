...
**Implementation**:
- `snippets/utility--css-variables.liquid` v1.14.1 (CSS variable emitter — `:root` `--color-<handle>` lines)
...
**Consumers**:
- `snippets/utility--css-variables.liquid` v1.14.1 — iterates `metaobjects.theme_color.values`, emits `--color-<handle>` in `:root`
...
- **Page**: `sections/theme-color.liquid` + `templates/page.theme-color.json`.
...
- `utility--css-variables.spec.md` — the substrate emitter that materializes `theme_color` entries into `--color-<handle>` declarations + the per-scheme `--color-role-*` namespace
- `utility--color-contrast.spec.md` — sibling utility consuming `hex_code.value` to pick legible foreground on arbitrary backgrounds
- `gradient.spec.md` — sibling metaobject; gradient stops reference `--color-role-*` (scheme-driven), not `--color-<handle>` (palette), so the two namespaces stay disjoint
