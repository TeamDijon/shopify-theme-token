...
- `sections/section.liquid` v1.9.1 (`block_rhythm` setting consumer — emits `--block-rhythm: var(--spacing-<picked-handle>)` per section)
...
- `sections/section.liquid` v1.9.1 — `block_rhythm` setting (metaobject picker) drives `--block-rhythm: var(--spacing-<handle>)` per-section
- `blocks/spacer.liquid` v2.0.0 / `snippets/spacer.liquid` v2.0.0 — `size` setting (metaobject picker) drives the spacer height: `--spacer-block-size: var(--spacing-<handle>)`
...
The substrate defaults exposed by `layer-base.css` (`--spacing-xs/sm/md/lg/xl`) are documented in `design-constants.spec.md`. This spec doesn't duplicate them — the metaobject's job is the override / extension surface on top of those defaults.
...
- **Per-block top-spacing is *not* metaobject-driven.** Every block carries `mobile_margin_block_start` / `desktop_margin_block_start` as raw px ranges (not a `spacing` picker), emitted as rem via the standard `divided_by: 16.0 | round: 3` conversion (per `utility--block-layout-vars.spec.md`). Per-block margins are instance-specific *absolute* overrides that replace the section's rhythm for a given block (range `0…100`, positive only; not added on top) — the section sets the baseline, a block opts out individually with an exact value. Forcing per-block margins through the metaobject would explode the catalog or over-constrain merchants.
...
- **CSS emission mechanics** — covered by `utility--css-variables.spec.md`. This spec describes the data contract + runtime semantics; that spec covers how the snippet composes the spacing emission alongside the four other domains.
- **Substrate spacing defaults** — `layer-base.css` seeds `--spacing-xs/sm/md/lg/xl` in rem. The substrate constants live in `design-constants.spec.md`. This spec is the override / extension surface on top of those defaults.
- **Block-rhythm cascade rule** — the CSS rule (`[data-modifiers*='theme-root'] > .shopify-block:not(:first-child) { margin-block-start: var(--mobile-margin-block-start, var(--block-rhythm, 0rem)); }`) lives in `layer-theme.css` and is covered in `section.spec.md` + `theme-root.md`. This spec describes only the spacing data layer.
...
- `design-constants.spec.md` — substrate-side spec covering `--spacing-xs/sm/md/lg/xl` defaults in `layer-base.css`. The dual-emission cascade-override pattern is documented there + here.
- `utility--css-variables.spec.md` — the substrate emitter that materializes the metaobject loop. Defers the data contract + cascade-override semantics to this spec.
- `section.spec.md` — primary block-rhythm consumer; emits `--block-rhythm: var(--spacing-<picked-handle>)` per its `block_rhythm` setting.
