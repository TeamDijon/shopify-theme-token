...
- **Manager class internals** — each of `EventsManager` / `ObserversManager` / `CacheManager` / `ModifiersManager` ships its own contract. `modifiers-manager.spec.md` exists; the other three are deferred per the substrate audit (`Defer until adjacent work forces it`).
- **`<token-section>` Liquid emission** — covered by `section.spec.md` (the section host spec). This spec covers the JS class and the element registration; the Liquid that writes the tag belongs to its consumer.
...
- **`<token-layout>`** — separate custom element (`assets/token-layout.js`, the inner-wrapper anchor for container blocks). Empty class, no manager wiring; doesn't extend `BaseComponent`. See `group.spec.md` / `columns.spec.md` for its consumer contract.
...
- **SSR / hydration** — Shopify renders sections via Liquid in HTTP responses; there's no hydration step. Custom elements upgrade on parser visit. **Deferred hydration** (island-style — wait until the element is visible / idle before triggering subclass setup) is opt-in per subclass via the utility documented in `hydrate.spec.md` (planned). The base class stays eager (default browser custom-element upgrade); subclasses with expensive setup wrap their `connectedCallback` work in `hydrateOnVisible(...)` to defer until the element enters the viewport. No base-class API change.

## Related

- `modifiers-manager.spec.md` — sibling JS spec; the manager `BaseComponent` lazy-instantiates with `this` binding for every theme custom element.
...
- `section.spec.md` — the section host spec; canonical consumer of `<token-section>`.
...
- `hydrate.spec.md` — planned substrate utility-js spec for opt-in deferred hydration (`hydrateOnVisible`); BaseComponent stays eager, subclasses opt in per their own connectedCallback.
