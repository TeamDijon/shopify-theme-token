...
**Depends on**:
- `var(--focus-ring-width)`, `var(--focus-ring-offset)`, `var(--color-role-focus-ring)` (CSS custom properties from `design-constants.spec.md` + `color-scheme.spec.md`) — consumed by the `:focus-visible` rule
...
- `design-constants.spec.md` — sources `--focus-ring-width`, `--focus-ring-offset`, and the cascade-layer declaration (`@layer base, reset, theme, utilities`)
- `color-scheme.spec.md` — sources `--color-role-focus-ring`
- `layer-theme.spec.md` — sibling substrate stylesheet (`@layer theme`); positionally beats `reset` in cascade order
