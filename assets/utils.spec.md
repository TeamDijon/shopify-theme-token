...
- `base-component.spec.md` — uses none of these directly today; future consumers (specialized sections from Bucket B) will compose `throttle` / `debounce` around event handlers
- `cache-manager.spec.md` — sibling substrate utility-js spec; covers element-scoped memoization (different concern from these pure utilities)
- `dom.spec.md` — sibling substrate utility-js with DOM-level lazy getters
- `core.spec.md` — re-exports these utilities to `window.Token.utils.*` for inline-script consumers
