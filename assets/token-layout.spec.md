...
- `base-component.spec.md` — the sibling substrate custom-element class; `<token-section>` is registered there, `<token-layout>` here
- `core.spec.md` — entry point that imports `@theme/token-layout` and triggers the global registration as a side effect
