...
**Depends on**:
- `var(--layer-temporary)` (z-index scale entry from `design-constants.spec.md`) — consumed by the `.skip-to-content:focus-within` rule
- `var(--duration-slow)`, `var(--ease-standard)` (motion scales from `design-constants.spec.md`) — consumed by the `<details>` animation block
...
- Any element bearing `data-modifiers*="locked-scroll"` on `<html>` — the `documentScroll` lock pattern (see `document-utils.spec.md`)
...
When `<html data-modifiers*="locked-scroll">` is active (the JS-side `documentScroll.lock()` from `document-utils.js`), the document position-fixes to suppress scroll without losing inline-size. Coordinated with JS-side `top: -<scrollY>px` mutation per `document-utils.spec.md` § documentScroll.
...
- `design-constants.spec.md` — sources `--layer-temporary`, `--duration-slow`, `--ease-standard`
- `layer-reset.spec.md` — sibling substrate stylesheet; the `interpolate-size: allow-keywords` opt-in on `html` lives there
- `skip-to-content.spec.md` — the snippet using `.skip-to-content`
- `icon.spec.md` — uses the `svg[data-name]` selector chain
- `richtext.spec.md` — primary consumer of the prose modifier
- `document-utils.spec.md` — JS-side counterpart to scroll-lock + sr-only patterns
...
- `utility--core-assets.spec.md` — captures this file as part of stage-1 inline emission
