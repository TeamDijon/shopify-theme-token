...
**Consumers**:
- `assets/base-component.js` v1.2.0 — every `<token-*>` custom element exposes a lazy `.observers` getter; `disconnectedCallback` calls `.clear()`. Full contract in `base-component.spec.md`.
...
- `base-component.spec.md` — the primary consumer; describes the lazy-getter / `disconnectedCallback` lifecycle that drives `.clear()`
- `events-manager.spec.md` — sibling substrate utility-js spec; identical lifecycle pattern around `addEventListener` / `removeEventListener`
- `cache-manager.spec.md` — sibling substrate utility-js spec (calibration reference); the no-sugar design v3.0.0 mirrors
