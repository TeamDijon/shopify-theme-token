...
- **Section inter-block rhythm flows through `--block-rhythm`.** Sections set `--block-rhythm: var(--spacing-<picked-handle>)` once (responsive resolution baked into the spacing token's `@media` branch), and the centralized rule in `layer-theme.css` consumes it as a fallback when no per-block top spacing is authored. Reach for a `spacer` block only when you need a one-off gap larger than the rhythm — not as a default spacing tool between adjacent blocks.
