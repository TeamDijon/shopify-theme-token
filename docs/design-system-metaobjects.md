...
| `gradient` | — (iterated via `metaobjects.gradient.values`) | `.system.handle`, `.angle.value`, `.color_start.value`, `.color_end.value` | Scheme-adaptive linear gradient; consumed by `utility--css-variables` → `--gradient-<handle>`, with stops referencing per-scheme `--color-<role>` so it re-resolves per scheme. Handle `background` reserved (scheme background-gradient) |
