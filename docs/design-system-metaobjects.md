...
| `media_size` | individual reference per media block | `.system.handle` (for the `fill` special), `.type.value` (`ratio` / `relative` / `fixed`), `.value.value` (CSS value: `16/9`, `100svh`, `400px`) | Sizing constraint for a media block; spec at `Liquified.dev/docs/media-block-spec.md` |
