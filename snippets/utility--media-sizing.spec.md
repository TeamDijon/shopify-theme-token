...
- `media-size.spec.md` — the `media_size` metaobject schema (modes, value field type-per-mode, seeded `fill` handle)
- `media.spec.md` — primary consumer (block CSS keyed off `sizing:<class>`)
- `embed.spec.md` — primary consumer (same modifier surface, different inner-content rules)
- `utility--dynamic-style.spec.md` — the scoping wrapper consumers route the `vars`-mode output through
