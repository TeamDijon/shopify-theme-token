...
- **Source**: `snippets/button.validation.json` — colocated fixture matrix, generated into the generic `?view=validation` slot (`sections/validation.liquid` harness + a generated, gitignored `templates/index.validation.json`) for the run, then dropped. Not shipped.
- **Tests**: `snippets/button.test.js` (executable; `npm run test:e2e`)
- **Requires seeded**: `icon/arrow`, `button_style/link-primary`, `button_style/outline-primary`, `content_width/reading` — Token's shipped seed catalog (see `icon.spec.md`, `button-style.spec.md`, `content-width.spec.md` § Seed entries). A test needing an unseeded handle signals a seed-set gap, not a test workaround.
...
- **Assertions** (executable — `snippets/button.test.js`):
...
- `button-style.spec.md` — the metaobject driving the variant catalog this block consumes via `[data-modifiers*='button-style:<handle>']` selectors
- `icon.spec.md` — the optional leading-icon consumer; reads `icon` metaobject reference passed via the block's setting
- `utility--block-layout-vars.spec.md` — emits the three per-instance vars (`--content-width`, `--mobile-margin-block-start`, `--desktop-margin-block-start`) this block consumes
