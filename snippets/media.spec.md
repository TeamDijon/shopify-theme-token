...
`container_style` variant rules live centrally in `layer-theme.css` `@layer theme`, scoped via `:where(.shopify-block--group, .shopify-block--columns, .shopify-block--media)[data-modifiers*='container-style:<handle>']`. The variant CSS for `card` / `outlined` / `elevated` doesn't live in this snippet — see `container-style.spec.md`.
...
- Group spec (sibling flex container; share container_style + color_scheme + bleed conventions): `group.spec.md`
- Container-style spec (centralized variant CSS in `layer-theme.css`): `container-style.spec.md`
- Subgrid migration (strict container-only bleed, named-line grid): `.context/docs/subgrid-migration.md`
- Image spec (renderer for `media_type: image`): `image.spec.md`
- Video spec (renderer for `media_type: video`): `video.spec.md`
