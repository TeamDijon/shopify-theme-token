...
- **Mutation API.** The utility emits the attribute at render time only; runtime mutation of `data-modifiers` happens via `ModifiersManager` on the JS side per `modifiers-manager.spec.md`. The Liquid utilities are write-once at render.
...
- `modifiers-manager.spec.md` — JS-side mutation API; reads and writes the same attribute this utility emits
- `document-utils.spec.md` — `documentModifiers` singleton, the document-scope JS counterpart to `utility--document-modifiers`
- `section.spec.md` — outlier consumer; emits `data-modifiers` inline rather than through this utility
