...
- `utility--block-layout-vars.spec.md` — the canonical L1-block consumer; emits the three shared per-instance vars (`--content-width`, `--mobile-margin-block-start`, `--desktop-margin-block-start`) through this utility's scoping wrapper
- `.context/docs/asset-loading.md` — inline-CSS routing through `utility--asset-loader` (the downstream this utility feeds)
- `section.spec.md` — section-layer consumer; emits `--content-width` + `--block-rhythm` per-section via this utility
