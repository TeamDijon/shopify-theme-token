...
- `content-width.spec.md` — the metaobject this utility reads via the `content_width` arg
- `spacing.spec.md` — the metaobject that drives section-level `--block-rhythm`; the per-block margin override pair (this utility's job) acts as the override layer atop the section's rhythm
- `utility--dynamic-style.spec.md` (planned) — the snippet that scopes the captured CSS to `#shopify-block-<id>`
- `section.spec.md` — describes the section-level emission that the per-block override layer sits on top of
