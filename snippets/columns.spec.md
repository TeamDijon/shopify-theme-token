...
`container_style` variant rules live centrally in `layer-theme.css` (shared with `group` and `media`). See `container-style.spec.md`.
...
- Group spec (sibling flex container; share outer/inner architecture, container_style, color_scheme, bleed conventions): `group.spec.md`
- Media spec (sibling container block specialized for media surfaces): `media.spec.md`
