...
- **Product schema fields beyond price.** `og:availability` / `og:product:condition` / variant-aware OG are not emitted. Structured-data (JSON-LD) is the canonical surface for richer product metadata — see `utility--structured-data.spec.md`.
...
- `utility--structured-data.spec.md` — sibling SEO emitter; JSON-LD shape is richer for product / article metadata
- `layout.spec.md` — head spine stage 10 consumer
