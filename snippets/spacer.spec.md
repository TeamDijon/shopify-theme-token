...
- `section.spec.md` — describes the block-rhythm cascade that spacer's `margin-block-start` inherits from
- `spacing.spec.md` — the metaobject backing the `size` setting; defines the token scale + mobile/desktop pair
- `theme-color.spec.md` — `background_color` setting reads the metaobject for the optional colored band
