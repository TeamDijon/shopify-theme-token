...
- `snippets/image.liquid` (responsive image renderer; shipped — see `image.spec.md`)
...
- `image.spec.md` — responsive image primitive consumed for the media region
- `badge.spec.md` — sibling primitive consumers compose for category / tag pills on the card
...
- `design-constants.spec.md` — `--radius-default`, `--duration-slow`, `--ease-out` consumed for the card visuals
