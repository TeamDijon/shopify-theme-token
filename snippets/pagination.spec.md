...
- Hover uses `rgb(from)` syntax for the subtle background tint (matches the substrate's transparency vocabulary per `utility--css-variables.spec.md`).
...
- `button.spec.md` — possible consumer (if load-more uses the button primitive for visual consistency)
...
- `utility--css-variables.spec.md` — `rgb(from)` transparency syntax used in the hover state
