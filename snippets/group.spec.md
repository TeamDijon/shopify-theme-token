...
`container_style` variant rules live **centrally in `layer-theme.css` `@layer theme`**, not in this snippet — they're shared across `group`, `columns`, and `media` via a `:where(...)` selector that doesn't carry to this file. See `container-style.spec.md` for the variant catalog and the per-project extension contract.
