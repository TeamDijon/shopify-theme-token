...
- `design-constants.spec.md` — `--duration-*`, `--ease-*` (for body transitions consuming these tokens), `--radius-*` (consumers compose with role tokens for variant CSS)
...
- **Disjoint namespace from `theme_color`.** `--color-role-<role>` (scheme-assigned, per-section role) and `--color-<handle>` (palette-flat, scheme-independent) are deliberately disjoint. A `theme_color` entry handled `background` emits `--color-background` and does not interact with `--color-role-background`. See `theme-color.spec.md` Behavior for the contract from the palette side.
...
- **Focus-visible color via `--color-role-focus-ring`.** The focus ring uses an opaque alias of `--color-role-primary` — never a translucent token. Pairs with `design-constants.spec.md`'s `--focus-ring-width` (`0.125rem`) and `--focus-ring-offset` (`0.125rem`) for the full focus-visible style.
...
- **`theme_color` palette tokens (`--color-<handle>`).** Owned by `theme-color.spec.md`. Scheme-independent; do not interact with the role-token namespace.
- **Gradient catalog (`--gradient-<handle>`).** Owned by `gradient.spec.md`. The `--gradient-background` token in this spec is scheme-driven (one per scheme); the gradient catalog's named entries are scheme-adaptive via role-token interpolation but lives in a separate namespace.
- **Text-style typography tokens (`--<handle>-font-*` + `--base-*`).** Owned by `text-style.spec.md`. Body-level transitions in `layer-theme.css` consume `--base-*` independently of the role-token system.
...
- **Custom-property emission shape.** Owned by `utility--css-variables.spec.md` — this spec describes the role-token contract; that spec describes how the emission integrates with the other five emission domains (palette, gradients, text-style, gutter, spacing).

## Related

- `theme-color.spec.md` — sibling substrate metaobject spec; the `--color-<handle>` palette catalog. Explicitly defers the role-token system to this spec (its Out of scope cross-references); the two namespaces are deliberately disjoint
- `utility--css-variables.spec.md` — the emitter that materializes this spec's contract; the per-scheme rule block lives there. This spec owns the contract; that spec owns the emission shape across all six domains
- `gradient.spec.md` — sibling substrate metaobject; gradient stops reference `--color-role-<role>` from this namespace
- `design-constants.spec.md` — provides `--duration-*` / `--ease-*` for body transitions consuming these tokens; `--radius-*` for variant CSS
- `layer-theme.spec.md` — the consumer aggregator stylesheet; body-level appearance + form-input states + container-style variants all read from this namespace
