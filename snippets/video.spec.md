...
- Media spec (primary consumer; renders `<media-video>` inside its positioned wrapper; carries the `video_mode` / `video_controls` / `video_autoplay` / `video_loop` settings through): `media.spec.md`
- Image spec (sibling renderer for the `image` media_type branch; same art-direction pattern via `<picture>` / `<source media>`): `image.spec.md`
- Embed spec (third-party video alternative; same `media_size` sizing API): `embed.spec.md`
