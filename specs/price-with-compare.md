...
- `design-constants.md` — `--radius-pill` consumed for the save pill rounding
- `rating.md` — sibling L1 block planned (also consumes a small inline primitive); when product blocks land, the PDP composition pattern crystallizes
- `inventory-status.md` — sibling sub-component primitive (variant-level state pill); shares the JS update path pattern (variant change → re-read data-* + re-render)
