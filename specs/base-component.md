...
**Implementation**: `assets/base-component.js` v1.1.0 (class + custom-element registration)
...
- `assets/core.js` v1.2.0 — re-exports `BaseComponent` as an ES module export and as `window.theme.components.BaseComponent` for inline scripts and Liquid templates
...
- **`section` getter is not cached.** Each access calls `closest('.shopify-section')` fresh. Caching would risk staleness if the DOM is reshuffled — and `closest` is cheap. The lookup-time `console.warn` on miss is diagnostic-only: a component used outside a section is unusual and worth flagging.
...
- **No shadow DOM.** The class extends `HTMLElement` directly, not via `attachShadow`. Light-DOM children (the section's blocks) are addressable by Liquid, CSS, and outer-scope selectors. Specialized sections that genuinely need shadow DOM scope it in their own subclass — not the default.
- **No `observedAttributes` / `attributeChangedCallback`.** The base class doesn't observe attribute changes — `data-modifiers` mutations go through `ModifiersManager` writes directly. Subclasses needing attribute-reactive behavior add `static get observedAttributes()` and `attributeChangedCallback(name, old, new)` themselves.
...
window.customElements.define("token-cart", Cart);
```
...
- **Unit scope** (prose; Vitest specs once installed):
  ...
  - **`section` getter**:
    - With `.shopify-section` ancestor → returns the matched element.
    - Without `.shopify-section` ancestor → returns `null` and logs one `console.warn` (assert via `vi.spyOn(console, 'warn')`).
    - Not cached — wrapping/unwrapping the element in a `.shopify-section` parent between calls produces the expected match/miss without manual invalidation.
  - **Custom-element registration**:
...
- **`window.theme` namespace** — populated by `core.js`, not by this file. The base-component module exports the class; the namespace plumbing is the entry point's job.
...
- **SSR / hydration** — Shopify renders sections via Liquid in HTTP responses; there's no hydration step. Custom elements upgrade on parser visit. No SSR-specific surface in this spec.
