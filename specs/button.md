...
**Implementation**:
- `snippets/button.liquid` v1.5.1 (render surface)
- `blocks/button.liquid` v1.5.0 (block schema + render call)

**Reconciled**: 2026-06-29 (snippet v1.5.1 — add `justify-self: start` so a bare button in a section grid hugs its content instead of filling the track: a grid item blockifies `inline-flex` → block-level `flex`, which filled the track regardless of `full_width: none`, silently breaking that setting's "content width" contract. Surfaced by the content Tier-3 page. Block v1.5.0 unchanged.)
...
- **Content-sized as a grid item.** The button is `display: inline-flex` (content-sized). As a *direct grid item* — a bare button in a section's theme-root grid — CSS blockifies it to block-level `flex`, which would fill the track. `justify-self: start` keeps it content-sized + start-aligned, so `full_width: none` reads as content-width in a grid, not only in flex. `full-width:*` overrides with `inline-size: 100%` and fills regardless. In a flex parent (`group` / `<media-contents>`) `justify-self` is ignored — the container's alignment governs (the button is a flex item that already hugs its content). Arrange or re-align multiple buttons via a `group`.
