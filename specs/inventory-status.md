...
- `price-with-compare.md` — sibling sub-component primitive (variant-driven, JS update path); shares the data-attributes + future JS module pattern
- `theme-color.md` — `--color-success` / `-warning` / `-info` palette tokens consumed for state colors (the semantic seed entries per `metaobject-definitions.md` § theme_color)
...
- `icon.md` — for the decorative-icon prefix consumers may compose alongside the status pill
