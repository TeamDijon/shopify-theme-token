...
- `snippets/utility--css-variables.liquid` v1.13.0 — overrides matching spacing handles with `spacing` metaobject values (mobile in `:root`, desktop in nested `@media`); cascade position drives the override
...
- **Spacing scale uses rem; metaobject overrides emit px.** Substrate rem values scale with root font-size — components default to user-typography-relative spacing. The `spacing` metaobject's `mobile_value` / `desktop_value` fields are px integers (existing schema); the override emission preserves that unit. The mixing of units across the catalog is visible to anyone debugging computed styles but functionally fine — both resolve to pixel values. A merchant who wants typography-relative spacing keeps the substrate defaults; a merchant who sets metaobject entries gets fixed px regardless of root font.
...
- **Spacing responsiveness propagates through `var()`.** The metaobject emits `--spacing-<handle>: <mobile>px` in `:root` and `--spacing-<handle>: <desktop>px` in nested `@media (width >= 48rem)`. Consumers reading `var(--spacing-md)` resolve responsively at use-site (CSS variables are substituted at used-value time per spec). A per-section `--block-rhythm: var(--spacing-<handle>)` propagates the responsive resolution through the chain — layer-theme.css reads `var(--block-rhythm)` once, gets the right value per breakpoint.
...
- **Assertions** (prose; Playwright once installed):
  ...
  - Computed `--spacing-md` on `:root` equals `1rem` when no `md`-handled spacing metaobject entry exists; equals the metaobject's `mobile_value` (px) at viewport `< 48rem` and `desktop_value` (px) at viewport `>= 48rem` when an entry exists. Verifiable via `getComputedStyle(document.documentElement).getPropertyValue('--spacing-md')`.
