...
**Implementation**:
- `snippets/button.liquid` v1.4.0 (consumer + stylesheet host — appends `button-style:<handle>` to `data-modifiers`; the snippet's `{% stylesheet %}` block carries per-handle CSS rules targeting `[data-modifiers*='button-style:<handle>']`)
...
**Consumers**:
- `snippets/button.liquid` v1.4.0 — primary consumer; reads the `button_style` setting from the button block, appends `button-style:<handle>` to `data-modifiers`, and applies the matching `{% stylesheet %}` rule
