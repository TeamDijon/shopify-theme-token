...
- **`dom.pageContent`** (from `document-utils.js`) — lazy-getter returns `document.getElementById('page_content')`. Currently the only entry in the `dom` catalog (per `dom.md` v2.0.0).
...
Neither layout currently registers a SPA / `view-transition` opt-in. The `@view-transition { navigation: auto; }` cross-document view-transition declaration in `layer-base.css` (per `design-constants.md`) is the substrate's opt-in; layouts don't need a per-layout declaration.
...
- **Edge cases**:
  ...
  - FR locale active → `<html lang="fr">`, hreflang alternates change accordingly (per `utility--hreflang.md`)
...
- `.context/specs/section.md` — the next inner wrapper inside `content_for_layout`; merchant-composable sections render between layout's `<main>` and the L1 blocks
- `.context/specs/dom.md` — `dom.pageContent` getter; depends on `id="page_content"` on `<main>`
- `.context/specs/document-utils.md` — `documentModifiers` (consumes the `<html data-modifiers>` attribute emitted via `utility--document-modifiers`)
- `.context/specs/design-constants.md` — substrate constants the `<head>` pipeline indirectly emits via `utility--core-assets`
