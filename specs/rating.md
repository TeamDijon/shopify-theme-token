...
- Locale keys are owned by `star-rating.md` (`accessibility.star_rating.*`); this block adds none.
...
| `mobile_margin_block_start` | range (-200–200, step 2, px) | no | `0` | Standard top-spacing setting; routes through `utility--block-layout-vars` → `--mobile-margin-block-start`. |
...
- `star-rating.md` — L0 snippet this block composes; owns the visual rendering + accessible label
...
- `title.md`, `button.md` — sibling L1 blocks consuming similar block-level color-scheme + top-spacing patterns
