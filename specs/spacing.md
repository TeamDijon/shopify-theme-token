...
- `sections/section.liquid` v1.9.0 (`block_rhythm` setting consumer — emits `--block-rhythm: var(--spacing-<picked-handle>)` per section)
...
- `sections/section.liquid` v1.9.0 — `block_rhythm` setting (metaobject picker) drives `--block-rhythm: var(--spacing-<handle>)` per-section
