...
- `utility--css-variables.md` — reads `--color-role-*` (scheme tokens), `--base-*` (text-style aliases), `--gradient-background`, `--gutter`, `--content-width`, `--block-rhythm` (section-emitted)
- `utility--block-layout-vars.md` — reads per-instance `--mobile-margin-block-start` / `--desktop-margin-block-start` for the block-rhythm cascade
- `container-style.md` — handle catalog for the variant rules
...
`:where()` keeps specificity at zero so per-block stylesheets and per-project overrides can re-style without escalation. Three seeded variants (`card` / `outlined` / `elevated`); per-project extensions add their own variant rules following the same selector shape — see `container-style.md`.
...
N/A — this file consumes CSS variables from `design-constants.md` + `utility--css-variables.md` + `utility--block-layout-vars.md`. It doesn't define new variables.
...
- `prefers-reduced-motion` zeroes the body-level transition (`background-color`, `color`). Pairs with `design-constants.md`'s view-transition reduced-motion override.
- `:focus-visible` outline on form inputs uses `--color-role-focus-ring` (an alias of `--color-role-primary`) + `0.125rem` (= 2px) thickness with `0.125rem` offset — matches the focus-ring scale documented in `design-constants.md`.
...
- **Edge cases**:
  ...
  - Container-style handle without a matching variant rule → modifier emits, no variant styling applies, block renders with default chrome (diagnostic mode per `container-style.md`)
...
- **`@view-transition` opt-in + reduced-motion override.** Owned by `design-constants.md` (the at-rule lives in `layer-base.css`). This file's reduced-motion override is body-transition-specific.

## Related

- `.context/specs/design-constants.md` — sibling substrate CSS (`assets/layer-base.css`); provides the constants this file consumes (`--duration-*`, `--ease-*`, `--radius-default`)
- `.context/specs/utility--css-variables.md` — emits the `--color-role-*` scheme tokens, `--base-*` text-style aliases, `--gradient-background`, `--gutter`, `--spacing-*` that this file consumes
- `.context/specs/utility--block-layout-vars.md` — emits the per-instance `--mobile-margin-block-start` / `--desktop-margin-block-start` / `--content-width` that the bleed-grid + rhythm cascade consume
- `.context/specs/container-style.md` — handle catalog for the variant rules (`card` / `outlined` / `elevated` + per-project extensions)
