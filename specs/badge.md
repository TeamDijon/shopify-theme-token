...
- Semantic `theme_color` palette seeds (`success`, `warning`, `error`, `info`) — emit as `--color-<handle>` per `theme-color.md`. The spec assumes these handles are seeded per `metaobject-definitions.md` § theme_color recommended entries.
...
- `icon.md` — leading-icon source; consumers pass `file_name` directly to `badge`, which forwards it
- `inventory-status.md` — sibling tone-colored sub-primitive; uses similar per-state color resolution via `data-modifiers`; styled as a tone-colored inline label rather than a padded pill
- `theme-color.md` — semantic palette tokens (`--color-success` / `-warning` / `-error` / `-info`) consumed for the four semantic tones
- `design-constants.md` — `--radius-small` consumed for the badge corner rounding
- `.context/docs/modifier-system.md` — `data-modifiers` convention; this snippet emits `tone:<value>,badge-style:<value>`
- `.context/specs/utility--css-variables.md` — color-mix vocabulary (used here for tint formula); the substrate's transparency vocabulary (`rgb(from)`) is the sibling — different intents
