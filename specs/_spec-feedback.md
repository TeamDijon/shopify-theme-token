...
| `convention:codebase-wide-emission-audit-on-metaobject-review` | 1 | 2026-06-04 / 2026-06-04 | `spec-review` skill (checklist) or `spec-convention.md` (metaobject variant guidance) | When reviewing a metaobject spec whose Behavior section describes the emission unit (`px` / `rem` / etc.), audit the snippet against sibling emitters for unit consistency. Surfaced in spacing review (M2): metaobject emitted px while every sibling emitter in the same file emitted rem — the inconsistency was documented in Behavior as a deliberate design choice but was actually a drift. A standing audit step would catch this proactively. |
