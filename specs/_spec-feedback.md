...
| `convention:codebase-wide-emission-audit-on-metaobject-review` | 2 | 2026-06-04 / 2026-06-04 | `spec-review` skill (checklist) or `spec-convention.md` (metaobject variant guidance) | When reviewing a metaobject spec whose Behavior section describes the emission unit (`px` / `rem` / etc.), audit the snippet against sibling emitters for unit consistency. Surfaced 2× across 2026-06-04 substrate-metaobject batch: spacing review M2 (metaobject emitted px while every sibling emitted rem) and content-width review M1 (same px-vs-rem inconsistency, plus a sibling utility-snippet spec carried an outdated rationale for the inconsistency). Pattern is recurring — strong promotion signal at the next periodic review. |
...
| `convention:implementation-pin-version-audit` | 1 | 2026-06-04 / 2026-06-04 | `spec-review` skill (checklist item) | When closing a spec review, audit the Implementation pin against `spec-convention.md`'s `path/to/file vX.Y.Z (role)` format. Specs sometimes ship without version pins on consumer references (common drift for metaobject specs where the implementation is "the consumers"). Surfaced in media-size review: 3 consumer references lacked version pins; no marker would have caught it without a standing audit step. Adding to the spec-review checklist prevents the drift from accumulating. |
