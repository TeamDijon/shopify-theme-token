...
| `voice:backlog-references-as-residue` | 1 | 2026-06-04 / 2026-06-04 | `reference-voice.md` Version-history-references rule (recently promoted) | Extend the version-history-references rule to explicitly cover BACKLOG / TODO / triage-doc references in spec bodies. BACKLOG is a transient queue artifact — what's important about a deferred feature is its contract (out of scope, what would activate it), not its current position in the BACKLOG taxonomy. Surfaced in button-style review (M6 "BACKLOG-noted 'link L1 block isn't needed' resolution" + unmarked "BACKLOG Bucket C (C9, surfaced from button spec retrofit 2026-05-29)"). Two sites in one spec; similar shape to v-history residue. Could be a sibling bullet or a merged "Transient-artifact references" framing covering both. |
