...
**Implementation**:
- `snippets/media.liquid` v1.4.1 (consumer — reads the picked entry's `type` + `value` to emit `aspect-ratio` / `block-size` / `block-size: 100%` per the mode dispatch)
...
**Consumers**:
- `snippets/media.liquid` v1.4.1 — primary consumer; reads the picked `media_size` entry from the media block, dispatches on `type` (or routes to the special `fill` handle) to emit the appropriate CSS via dynamic style
