...
**Implementation**: `snippets/utility--block-layout-vars.liquid` v1.0.0 (CSS variable emitter — captured by the caller's `dynamic_style` block and routed through `utility--dynamic-style`)
...
- `snippets/button.liquid`, `snippets/columns.liquid`, `snippets/embed.liquid`, `snippets/group.liquid`, `snippets/media.liquid`, `snippets/richtext.liquid`, `snippets/separator.liquid`, `snippets/spacer.liquid`, `snippets/title.liquid` — 9 shipped L1 blocks each render this utility inside their `{% capture dynamic_style %}` block to emit per-instance vars
...
|---|---|---|---|---|
| `content_width` | metaobject (`content_width`) | no | blank | When present, emits `--content-width: <px>px` from the entry's `.width.value`. Pass blank (or omit) to skip — useful when the block has its own width logic (e.g., richtext in narrow mode that's been migrated to `content_width` metaobject entries per the audit settlement). |
...
|---|---|---|
| `--content-width` | `<value>px` | `content_width.width.value` when the metaobject arg is set; omitted otherwise |
...
- **API surface** (matrix to exercise per consumer):
  - **Content-width pick + blank**: block with metaobject picker resolved → `--content-width: <px>px` emitted; blank picker → no declaration
...
- **Edge cases**:
  ...
  - Content_width metaobject with `width.value` blank → emits `--content-width: px` (malformed); CSS drops the declaration. Schema validation should prevent.
...
- **Assertions** (prose; Playwright once installed):
  - When all three args are set, the block's computed style includes all three custom properties with the expected px / rem values
...
- **Validation of metaobject reference shape.** The snippet trusts the block's setting schema. A merchant-broken metaobject reference (e.g., GID points to a deleted entry) would emit `--content-width: px` and CSS drops it; the snippet doesn't pre-check.
