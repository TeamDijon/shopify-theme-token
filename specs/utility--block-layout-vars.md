...
- 8 of the 9 shipped L1 blocks each render this utility inside their `{% capture dynamic_style %}` block to emit per-instance vars: `snippets/button.liquid` v1.5.0, `snippets/columns.liquid` v1.8.1, `snippets/embed.liquid` v1.2.1, `snippets/group.liquid` v1.7.1, `snippets/media.liquid` v1.5.1, `snippets/richtext.liquid` v1.2.3, `snippets/separator.liquid` v1.0.5, `snippets/title.liquid` v1.1.5. `snippets/spacer.liquid` is the exception — it has no `content_width` or top-margin pair, emitting its own `--spacer-block-size` from the `spacing` token instead
